...
public class StrategyManager {
	...
	public void runBases(Game game, Player self)
	{
		...
		for(Base b : bases)
		{
			...
			else if(!shouldAttack && b.state == Information.BaseState.ATTACKING && b.getAllUnitsByType(Information.UnitType.MARINE).size() < 13)
			{
	...
	public void setBuildGoals(Player self, Game game)
	{
		...
		for(Base b : bases)
		{
			...
			if( game.self().supplyTotal() < 200 && game.self().supplyTotal() - game.self().supplyUsed() <= 4 + 2*totalBuildingCount(UnitType.Terran_Barracks) && canBuild(self, UnitType.Terran_Supply_Depot))
			{
				...
				if(!buildingManager.isQueued(UnitType.Terran_Supply_Depot))
				{
		...
		if(self.getUpgradeLevel(UpgradeType.U_238_Shells) == 0)
		{
			if(totalBuildingCount(UnitType.Terran_Barracks) >= 3 && totalBuildingCount(UnitType.Terran_Academy) > 0)
			{
				...
				for(Unit building : buildings)
				{
					...
					if(building.canUpgrade(UpgradeType.U_238_Shells))
					{
						System.out.println("Upgrading marine range");
	...
	public boolean shouldAttackNow()
	{
		...
		{
			return false;
		}else if(totalUnitCount(Information.UnitType.MARINE) >= 20 * bases.size())
		{
			return true;
		}
		else
