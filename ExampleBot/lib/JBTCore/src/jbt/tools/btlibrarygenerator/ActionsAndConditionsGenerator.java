			...
			if (actions.size() != 0) {
				try {
					...
					modelActionsOutputDirectory = Util
							.removeDirectorySeparator(modelActionsOutputDirectory.replace('/', '\\').trim());
					...
					executionActionsOutputDirectory = Util
							.removeDirectorySeparator(executionActionsOutputDirectory.replace('/', '\\').trim());
					...
					for (ParsedAction currentAction : actions) {
						try {
							/* Create model action. */
							String modelOutputFileName = modelActionsOutputDirectory.replace('/', '\\').trim()
									+ "\\" + currentAction.getName() + ".java";
			...
			if (conditions.size() != 0) {
				try {
					...
					modelConditionsOutputDirectory = Util
							.removeDirectorySeparator(modelConditionsOutputDirectory).replace('/', '\\').trim();
					...
					File executionConditionsDirectory = new File(executionConditionsOutputDirectory.replace('/', '\\').trim());
					...
					for (ParsedMethod currentCondition : conditions) {
						try {
							...
							String modelOutputFileName = modelConditionsOutputDirectory.replace('/', '\\').trim()
									+ "\\" + currentCondition.getName() + ".java";
							...
							String executionOutputFileName = executionConditionsOutputDirectory.replace('/', '\\').trim()
									+ "\\" + currentCondition.getName() + ".java";
